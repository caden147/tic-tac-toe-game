...
class TestMocking(unittest.TestCase):
    def test_can_send_messages_back_and_forth(self):
        ...
        try:
            ...
            client = Client('localhost', 9090, client_selector, client_logger, output_text_function=lambda x: client_output.append(x), socket_creation_function= lambda x: internet.create_socket_from_address(client_address, x))
            ...
            wait_until_true_or_timeout(lambda: len(client_logger.get_log(connection_handler.RECEIVING_MESSAGE_LOG_CATEGORY)) > 0)
            client.close()
            server.close()
